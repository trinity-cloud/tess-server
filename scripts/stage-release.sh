...
python3 - "$STAGE" "$NAME" "$VER" "$BID" "$CHANNEL" "$EXE_SHA" "$LIB_SHA" "$REPO" "$BINARY_MINOS" "$BINARY_SDK" << 'PY'
import datetime, glob, json, os, re, subprocess, sys
stage, name, ver, bid, channel, exe_sha, lib_sha, repo, actual_minos, actual_sdk = sys.argv[1:11]
...
components = [
    ...
    ("SPDXRef-Package-cpp-httplib", "cpp-httplib", "0.49.0", "MIT", "Copyright (c) 2017 yhirose", "LIBRARY"),
