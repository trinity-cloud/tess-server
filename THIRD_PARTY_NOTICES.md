...
Tess Server is a proprietary distribution built on open-source foundations. The following components are statically linked into or shipped with the `tess-server` binary. Their licenses travel with every copy; nothing in the Tess license limits your rights under them.
