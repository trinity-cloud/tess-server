...
test('loads all five source profiles with launcher mappings', async () => {
  const root = await resolveProfileRoot(packageRoot);
  const profiles = await loadProfiles(root);
  assert.equal(profiles.length, 5);
  assert.deepEqual(new Set(profiles.map(profile => profile.profile_id)), new Set(['dsv4-dspark', 'hy3-iq2m', 'minimax-m27-iq4xs', 'nemotron3-super-q4km', 'qwen36-a3b-q8-q4mtp']));
